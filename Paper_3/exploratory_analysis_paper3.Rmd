...
rm(list=ls())
name <- function(x) { as.data.frame(names(x))}
a <- read.csv("~/Documents/SEM2DSM1/Paper_3/data/accuracy.csv")
...
ME <- ggplot(a, aes(x = model, y = ME, fill = ME)) +
  geom_bar(stat = "identity", show.legend = F) +
  facet_grid(hor ~ sp) + 
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
  scale_fill_gradient2(low="#4885ed", mid = "#888888",  high="#db3236" ) + 
  scale_x_discrete(name="ME") + scale_y_continuous(name = "")


RMSE <- ggplot(a, aes(x = model, y = RMSE, fill = RMSE)) +
  geom_bar(stat = "identity", show.legend = F) +
  facet_grid(hor ~ sp) + 
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) + 
  scale_fill_gradient2(low="#4885ed", mid = "#888888",  high="#db3236" ) + 
  scale_x_discrete(name="RMSE") + scale_y_continuous(name = "")

AVE <- ggplot(a, aes(x = model, y = AVE, fill = AVE)) +
  geom_bar(stat = "identity", show.legend = F) +
  facet_grid(hor ~ sp) + 
  theme(axis.text.x = element_text(angle = 90, hjust = 1), axis.title.x = element_text(NULL)) + 
  scale_fill_gradient2(low="#4885ed", mid = "#888888",  high="#db3236" ) + 
  scale_x_discrete(name="AVE") + scale_y_continuous(name = "")

grid.arrange(ME, RMSE, AVE, ncol=3)
