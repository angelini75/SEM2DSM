...
# Extrapolation models from Argentina to Kansas
MM = mathematical model

CM = conceptual model

mM = model based on the mean of soil properties per horizons

## Model 1: SEM MM 
```{r Arg2Ks MM, message=FALSE, include=FALSE}
...
'
# Model calibration ####
my.fit.lv.ML <- sem(model = my.model.lv,data = Arg, meanstructure = FALSE, 
                    fixed.x = T)

# Model evaluation ####
summary(my.fit.lv.ML, fit.measures=TRUE, rsquare = F)
mod.Arg <- modindices(my.fit.lv.ML,sort. = T)
mod.Arg[mod.Arg$mi>3 & (mod.Arg$op == "~~"|mod.Arg$op == "~"),] 
...
d <- read.csv("KS.data-0.3.csv")[,c(-1)] #changed to version 0.3
name(d)
...
# Accuracy measures ####
# Residuals #
Res <- cbind(pre[,1], unstd(pre[,2:10], STt.ks[2:10,]), unstd(pre[,20:28],
                                                           STt.arg[2:10,]))
# # back transform OC
...
(RMSE.l <- sqrt(158/qchisq(1-0.025,158))*report$RMSE)
(RMSE.u <- sqrt(158/qchisq(  0.025,158))*report$RMSE)
...
Results
```{r, echo=F }
knitr::kable(report, caption = "Model 1", digits = 2)
knitr::kable(report2, caption = "Model 1", digits = 2)
```

Observed vs. predicted by soil property and horizon
```{r, include=F, echo=F}
# plot predicted vs observed | horizon and soil property
plot(xyplot(predicted ~ observed| hor, res[res$sp=="CEC",], 
            aspect= 1 , layout = c(1,3), main="CEC", 
            scales = list(limits = c(0,45)),
            panel = function(x, y) {
         panel.xyplot(x, y, pch = c(0,1,2), cex = 0.5, col="#555555")
         panel.abline(lm(y ~ x), col="#ff5555")
         panel.abline(0,1, col="#aaaaaa")
       }),
     position=c(0,0,0.5,1))
plot(xyplot(predicted ~ observed| hor, res[res$sp=="OC",], cex = 0.5,
                aspect=1, layout = c(1,3), main="OC", 
            scales = list(limits = c(0,4)),
            panel = function(x, y) {
         panel.xyplot(x, y, pch = c(0,1,2), cex = 0.7, col="#555555")
         panel.abline(lm(y ~ x), col="red")
         panel.abline(0,1, col="#aaaaaa")
       }),
     position=c(0,0,1,1), newpage = F)
plot(xyplot(predicted ~ observed| hor, res[res$sp=="clay",], cex = 0.5,
                aspect=1, layout = c(1,3), main="Clay", 
            scales = list(limits = c(0,70)),
            panel = function(x, y) {
         panel.xyplot(x, y, pch = c(0,1,2), cex = 0.7, col="#555555")
         panel.abline(lm(y ~ x), col="red")
         panel.abline(0,1, col="#aaaaaa")
       }), 
     position=c(0,0,1.5,1), newpage = F)
```

Observed vs. predicted by soil property
```{r, echo=F, include=T}
# plot predicted vs observed | soil property
plot(xyplot(predicted ~ observed| sp, res[res$sp=="CEC",], groups = hor, pch = c(0,1,2), cex = 0.7,
            aspect= 1 , layout = c(1,1), 
            scales = list(limits = c(0,45))),
     position=c(0,0,0.35,1))
plot(xyplot(predicted ~ observed| sp, res[res$sp=="OC",], groups = hor, pch = c(0,1,2), cex = 0.7,
            aspect= 1 , layout = c(1,1), 
            scales = list(limits = c(0,4))),
     position=c(0.35,0,0.695,1), newpage = FALSE)
plot(xyplot(predicted ~ observed| sp, res[res$sp=="clay",], groups = hor, pch = c(0,1,2), cex = 0.7,
            aspect= 1 , layout = c(1,1), 
            scales = list(limits = c(0,70))),
     position=c(0.7,0,1.05,1), newpage = FALSE)
```

```{r, include=FALSE }
t <- data.frame(SP = c(names(d)[2:10], "CEC", "OC", "Clay"),
                Model_1 = round(c(report$R2,report2$r2),3))
```

***********************************
## Model 2: SEM CM with respecifications 
```{r Arg2Ks CMwR, include=FALSE}
# Purpose        : Fit a SEM model with Argentinian data and apply in KS data
# Maintainer     : Marcos Angelini  (marcos.angelini@wur.nl); 
...
Results
```{r, echo=F}
knitr::kable(report, caption = "Model 2", digits = 3)
knitr::kable(report2, caption = "Model 2", digits = 3)
```
...
```{r, include=FALSE }
t$Model_2 <- round(c(report$R2,report2$r2),3)
```
################################################################################
***********************************
...
Results
```{r, echo=F }
knitr::kable(report, caption = "Model 3", digits = 3)
knitr::kable(report2, caption = "Model 3", digits = 3)
```
...
```{r, include=FALSE }
t$Model_3 <- round(c(report$R2,report2$r2),3)
```
...
Results
```{r, echo=F }
knitr::kable(report, caption = "Model 4", digits = 2)
knitr::kable(report2, caption = "Model 4", digits = 2)
```
...
```{r, include=FALSE }
t$Model_4 <- round(c(report$R2,report2$r2),3)
```
################################################################################
***********************************
## Model 5: MLR Arg to Kansas
```{r Arg2Ks MLR, include=FALSE}
# Purpose        : Fit a SEM model with Argentinian data and apply in KS data
# Maintainer     : Marcos Angelini  (marcos.angelini@wur.nl); 
...
#######################################
mod <- list(
  mod.cec.a = lm(CEC.A ~ dem + twi + vdchn + evisd + lstm + ndwi.a + ndwi.b, Arg),
  mod.cec.b = lm(CEC.B ~ dem + twi + vdchn + evisd + lstm + ndwi.a + ndwi.b, Arg),
  mod.cec.c = lm(CEC.C ~ dem + twi + vdchn + evisd + lstm + ndwi.a + ndwi.b, Arg),
  mod.oc.a = lm(OC.A ~ dem + twi + vdchn + evisd + lstm + ndwi.a + ndwi.b, Arg),
  mod.oc.b = lm(OC.B ~ dem + twi + vdchn + evisd + lstm + ndwi.a + ndwi.b, Arg),
  mod.oc.c = lm(OC.B ~ dem + twi + vdchn + evisd + lstm + ndwi.a + ndwi.b, Arg),
  mod.clay.a = lm(clay.A ~ dem + twi + vdchn + evisd + lstm + ndwi.a + ndwi.b, Arg),
  mod.clay.b = lm(clay.B ~ dem + twi + vdchn + evisd + lstm + ndwi.a + ndwi.b, Arg),
  mod.clay.c = lm(clay.C ~ dem + twi + vdchn + evisd + lstm + ndwi.a + ndwi.b, Arg)
)

# Model validation ####
#### Prediction ####
P <- cbind(ks[,1:10], matrix(nrow=1,ncol= 9, data = NA,
                           dimnames = list(NULL,paste0(names(ks)[2:10],".p"))))

for(i in 1:9){
  P[,i+10] <- predict.lm(mod[[i]], ks) 
}
...
for(i in 2:10){
  theta.MLR.mean[i] <- mean((R[,i]^2)/as.numeric(summary(mod[[i-1]])[6])^2)
  theta.MLR.median[i] <- median((R[,i]^2)/as.numeric(summary(mod[[i-1]])[6])^2)
}


P[2:10] <- unstd(P[,2:10], STt.ks[2:10,])
P[11:19] <- unstd(P[,11:19], STt.arg[2:10,])
R <- P[,1:10]
R[2:10] <- P[,2:10]-P[,11:19]
...
for (i in 2:10) {
  ...
  ME  <-  mean(P[,i] - P[,i + 9])
  # RMSE (root mean squared error)
  RMSE <- sqrt(mean((P[,i] - P[,i + 9]) ^ 2))
  MSE <- mean((P[,i] - P[,i + 9]) ^ 2)
  # SS (Sum of squares)
  SS <- sum((P[,i] - P[,i + 9]) ^ 2)
  # fill report table
  reportMLR[i-1,1] <- c(names(P)[i])
  reportMLR[i-1,2] <- ME
  reportMLR[i-1,3] <- RMSE
  reportMLR[i-1,4] <- SS
}
...
STt.ks <- as.data.frame(STt.ks)
STt.ks$SS <- NA 
for(i in seq_along(names(ks))){
  STt.ks$SS[i] <- sum(( d[i] - STt.ks$mean[i])^2)
}

reportMLR$R2 <- 1 - (as.numeric(reportMLR$SS) / as.numeric(STt.ks$SS[2:10]))
reportMLR
...
par(mfrow = c(1, 3), pty="s",mai=rep(0.7,4))
rsq<- NULL
CEC <- rbind(as.matrix(P[,c(2,11)]), as.matrix(P[,c(3,12)]),
             as.matrix(P[,c(4,13)]))
colnames(CEC) <- c("CECo","CECp")
rownames(CEC) <- 1:length(rownames(CEC))
CEC <- as.data.frame(CEC)
...
OC <- rbind(as.matrix(P[,c(5,14)]), as.matrix(P[,c(6,15)]),
            as.matrix(P[,c(7,16)]))
colnames(OC) <- c("OCo","OCp")
rownames(OC) <- 1:length(rownames(OC))
OC <- as.data.frame(OC)
...
clay <- rbind(as.matrix(P[,c(8,17)]), as.matrix(P[,c(9,18)]),
              as.matrix(P[,c(10,19)]))
...
# create report by soil property
report2.MLR <- data.frame(Soil_property = NA, ME = NA, RMSE = NA, r2 = NA)
z <- cbind(CEC, OC, clay)
for (i in c(1,3,5)) {
  # ME <- mean error 
  ME  <-  mean(z[,i] - z[,i + 1])
  ...
  MSE <- mean((z[,i] - z[,i + 1]) ^ 2)
  # fill report table
  report2.MLR[i,1] <- c(names(z)[i])
  report2.MLR[i,2:3] <- c(ME, RMSE)
}

report2.MLR$r2[1] <- rsq[1]
report2.MLR$r2[3] <- rsq[2]
report2.MLR$r2[5] <- rsq[3]

report2.MLR <- report2.MLR[c(-4,-2),]
report2.MLR
################################################################################

# reshape measured and predicted values for plotting
res <- rbind(as.matrix(P[,c(2,11)]), as.matrix(P[,c(3,12)]),
             as.matrix(P[,c(4,13)]))
res <- rbind(res, as.matrix(P[,c(5,14)]), as.matrix(P[,c(6,15)]),
             as.matrix(P[,c(7,16)]))
res <- rbind(res, as.matrix(P[,c(8,17)]), as.matrix(P[,c(9,18)]),
             as.matrix(P[,c(10,19)]))
...
Results
```{r, echo=F }
knitr::kable(reportMLR, caption = "Model 5", digits = 3)
knitr::kable(report2.MLR, caption = "Model 5", digits = 3)
```
...
```{r, include=FALSE }
t$Model_5 <- round(c(reportMLR$R2,report2.MLR$r2),3)
```
***********************************
## Model 6: MLR Kansas
```{r Ks MLR, include=FALSE}
...
# mod <- list(
#   mod.cec.a = lm(CEC.A ~ dem + twi + vdchn + evisd + lstm + ndwi.a + ndwi.b, ks),
#   mod.cec.b = lm(CEC.B ~ dem + twi + vdchn + evisd + lstm + ndwi.a + ndwi.b, ks),
#   mod.cec.c = lm(CEC.C ~ dem + twi + vdchn + evisd + lstm + ndwi.a + ndwi.b, ks),
#   mod.oc.a = lm(OC.A ~ dem + twi + vdchn + evisd + lstm + ndwi.a + ndwi.b, ks),
#   mod.oc.b = lm(OC.B ~ dem + twi + vdchn + evisd + lstm + ndwi.a + ndwi.b, ks),
#   mod.oc.c = lm(OC.B ~ dem + twi + vdchn + evisd + lstm + ndwi.a + ndwi.b, ks),
#   mod.clay.a = lm(clay.A ~ dem + twi + vdchn + evisd + lstm + ndwi.a + ndwi.b, ks),
#   mod.clay.b = lm(clay.B ~ dem + twi + vdchn + evisd + lstm + ndwi.a + ndwi.b, ks),
#   mod.clay.c = lm(clay.C ~ dem + twi + vdchn + evisd + lstm + ndwi.a + ndwi.b, ks)
# )
...
P[2:10] <- unstd(P[,2:10], STt.ks[2:10,])
P[11:19] <- unstd(P[,11:19], STt.ks[2:10,])
R <- P[,1:10]
R[2:10] <- P[,2:10]-P[,11:19]
...
Results
```{r, echo=F }
knitr::kable(reportMLR, caption = "Model 6", digits = 3)
knitr::kable(report2.MLR, caption = "Model 6", digits = 3)
```
...
```{r, include=FALSE }
t$Model_6 <- round(c(reportMLR$R2,report2.MLR$r2),3)
```
***********************************
## Model 7: mM Arg to Kansas

```{r Arg2Ks mM, include=FALSE}
# Purpose        : Fit a SEM model with Argentinian data and apply in KS data
# Maintainer     : Marcos Angelini  (marcos.angelini@wur.nl); 
...
# Model validation ####
#### Prediction ####
pre <- cbind(ks[1,], matrix(nrow=156,ncol= 9, data = NA,
                           dimnames = list(NULL,paste0(names(ks)[2:10],".p"))))
...
Results
```{r, echo=F }
knitr::kable(report, caption = "Model 7 (Arg mean model)", digits = 3)
knitr::kable(report2, caption = "Model 7 (Arg mean model)", digits = 3)
```
...
```{r, include=FALSE }
t$Model_7 <- round(c(report$R2,report2$r2),3)
```
################################################################################
***********************************
## Model 8: mM Kansas

```{r Ks mM, include=FALSE}
# Purpose        : Fit a SEM model with Argentinian data and apply in KS data
# Maintainer     : Marcos Angelini  (marcos.angelini@wur.nl); 
...
Results
```{r, echo=F }
knitr::kable(report, caption = "Model 8 (Ks mean model)", digits = 3)
knitr::kable(report2, caption = "Model 7 (Ks mean model)", digits = 3)
```
Observed vs. predicted by soil property and horizon
```{r, echo=F, include=F}
# plot predicted vs observed | horizon and soil property
...
```{r, include=FALSE }
t$Model_8 <- round(c(report$R2,report2$r2),3)
```
################################################################################

```{r}
knitr::kable(t, caption = "Model $r^{2}$ Cross-validation",digits = 2)
```
