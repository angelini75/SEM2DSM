...
# modification homosoil function

homotop <- function (grid_data,recipient.lon,recipient.lat) {
  #Gower’s similarity function
  gower <- function(c1, c2, r) 1-(abs(c1-c2)/r)
  #index global data
  grid.lon <-grid_data[,1] #longitude
  ...
  # homologue raster object
  r1 <- rasterFromXYZ(world.grid[, c(1, 2, 7)])
  r1 <- as.factor(r1)
  rat <- levels(r1)[[1]]
  rat[["homologue"]] <- c("", "homotop")
  levels(r1) <- rat
  
  retval <- list(r1, world.grid)
  return(retval)}

homolith <- function (grid_data,recipient.lon,recipient.lat) {
  #Gower’s similarity function
  gower <- function(c1, c2, r) 1-(abs(c1-c2)/r)
  #index global data
  grid.lon <-grid_data[,1] #longitude
  ...
  # homologue raster object
  r1 <- rasterFromXYZ(world.grid[, c(1, 2, 6)])
  r1 <- as.factor(r1)
  rat <- levels(r1)[[1]]
  rat[["homologue"]] <- c("", "homolith")
  levels(r1) <- rat
  
  retval <- list(r1, world.grid)
  return(retval)}

homoclime <- function (grid_data,recipient.lon,recipient.lat) {
  #Gower’s similarity function
  gower <- function(c1, c2, r) 1-(abs(c1-c2)/r)
  #index global data
  grid.lon <-grid_data[,1] #longitude
  ...
  # homologue raster object
  r1 <- rasterFromXYZ(world.grid[, c(1, 2, 5)])
  r1 <- as.factor(r1)
  rat <- levels(r1)[[1]]
  rat[["homologue"]] <- c("", "homoclime")
  levels(r1) <- rat
...
top <- homotop(grid_data = homosoil_globeDat,
                   recipient.lon = recipient.lon,
                   recipient.lat = recipient.lat)


# plot
area_colors <- c("#EFEFEF", "#666666")
levelplot(top[[1]], col.regions = area_colors,
          xlab = "", ylab = "") + 
  layer(sp.points(ba, col = "green", pch = 20, cex = 2))+ 
  layer(sp.points(ks, col = "green", pch = 20, cex = 2))


clime <- homoclime(grid_data = homosoil_globeDat,
               recipient.lon = recipient.lon,
               recipient.lat = recipient.lat)
# plot
area_colors <- c("#EFEFEF", "#666666")
levelplot(clime[[1]], col.regions = area_colors,
          xlab = "", ylab = "") + 
  layer(sp.points(ba, col = "green", pch = 20, cex = 2))+ 
  layer(sp.points(ks, col = "green", pch = 20, cex = 2))

lith <- homolith(grid_data = homosoil_globeDat,
                   recipient.lon = recipient.lon,
                   recipient.lat = recipient.lat)
# plot
area_colors <- c("#EFEFEF", "#666666")
levelplot(lith[[1]], col.regions = area_colors,
          xlab = "", ylab = "") + 
  layer(sp.points(ba, col = "green", pch = 20, cex = 2))+ 
  layer(sp.points(ks, col = "green", pch = 20, cex = 2))
