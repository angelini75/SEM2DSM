...
# 
# cv.fit<-fit.lmc(v = cv.var,g =  cv, fit.lmc = TRUE, fit.ranges = TRUE) 
...
# it is constant because we choose one single 
# range, sill and nugget for all cross variograms
levelplot(alpha.e[1:90,1:90]) 

# Let us call Rho = alpha * exp[-h/a]
Rho <- exp
for(i in seq_along(Rdist)) {
  if(exp[i] == 1){
    Rho[i] <- 1 # Then, Sigma0 is multiplied by 1
  } else {
