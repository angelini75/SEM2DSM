...
kronM <- function (RHO, RHO.I, SIGMA0, sp = 1:9, cov=10:18)#, make.dimnames = TRUE) 
{
  RHO <- as.array(RHO)
  ...
  for(i in cov){
    opobj[,,i,i] <- opobjRHO[,,i,i]
  }
  
  dp <- as.vector(t(matrix(1L:(2 * length(dRHO)), ncol = 2)[,1:2]))

  opobj1 <- aperm(a = opobj, dp)
  dim(opobj1) <- dSIGMA0 * dRHO
