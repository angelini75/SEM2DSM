...
vgm[[6]] <- vgm(nugget = 0,
                ...
                range=30000,
                model = "Exp")
vgm[[6]] <- fit.variogram(vg[[6]], vgm[[6]], fit.method = 7)
...
vgm[[9]] <- vgm(nugget = 15,
                ...
                range=200000,
                model = "Exp")
vgm[[9]] <- fit.variogram(vg[[9]], vgm[[9]], fit.method = 1)
plot(vg[[9]], vgm[[9]], main = "Clay.C")
attr(vgm[[9]], "SSErr")
vgm[[9]]

# Krige CV
x <- krige.cv(clay.C~1, R, vgm[[9]], nmax = 2, verbose = interactive())
x <- as.data.frame(x)
x$zscore[x$zscore == Inf] <- NA
x$zscore[x$zscore == -Inf] <- NA
x[is.na(x$zscore),1:5] <- -99
coordinates(x) <- ~X+Y
proj4string(x) <- NAD83.KS.N
library(mapview)
mapview(x, burst = F, zcol = "residual", cex = "zscore", legend = T)
bubble(x, "residual", main = "CEC.A residuals")
