...
corrgram(calib[calib$hor=="B",c(3:15,20:36)],lower.panel=panel.pie, upper.panel=panel.conf, diag.panel=panel.minmax)
...
setwd("/media/marcos/L0135974_DATA/UserData/BaseARG/2_Calibration")
write.csv(calib, "calib.data-4.4.csv")
...
# change all horizons to one row per location
# weighed average between same type of horizons (A1 and Ap = tickness*A1 +tickness*Ap = A) 
rm(list=ls()[ls()!="calib"])
name <- function(x) { as.data.frame(names(x))}

setwd("/media/marcos/L0135974_DATA/UserData/BaseARG/2_Calibration")
calib <- read.csv("calib.data-4.4.csv")[,-1]
########### Dictionary ###########
# calib = result of the previous line of this script. Each row is a horizon
#         with soil properties, standardised horizon name, soil profile number and
#         covariates
# covar = covariates extracted from calib with id, X and Y coordinates
# s = soil properties (all)
# n = s where horizons are A, B, BC or C.
# x are candidates for substitute missing C horizons
#################################

covar <- unique(calib[,c(1,21:38)])
s <- calib[,c(1,3:15)] #6,8,12,13)]
s$OC[1004] <- 0.1 #error en la base de datos
s <- s[s$id.p!=362,] #capas  del valle de inundacion del rio. Atipico
s <- s[s$id.p!=507,] #el contenido de arcilla parece ser = clay + silt 
# s.A <- s[s$hor == "A",]
# s.B <- s[s$hor == "B",]
# s.C <- s[s$hor == "C",]
# library(aqp)
...
# 
# 
# depths(s) <- id.p ~ top + bottom # data.frame to soil profile collection
# s
site <- unique(s$id.p)  
n <- s[s$hor== "A"| s$hor=="B" | s$hor=="C" | s$hor=="BC" ,]

#which profiles has NA in pH (it indicates NA in the entire hz.)
n[which(n$id.p %in% unique(n[which(is.na(n$phw)),1])),]

n$i <- 0
n$i[is.na(n$phw)] <- 1 
n[n$hor=="A",15] <- 0
n <- n[n$i==0,] # remove horizons without lab data, except for A hzs.
n <- n[,-15] 

n1 <- as.matrix(table(n[,c(1,2)])) # hz type per profile
n1 <- n1[n1[,1]!=0,]               # remove profiles without A hz
n2 <- n1[,c(5,6)]                  # hz BC or C
noC <- dimnames(n2)[[1]][n2[,2]==0]
n3 <- merge(as.data.frame(noC), n, by.x = "noC", by.y = "id.p", all.x = T)
n3 <- n3[n3$hor=="BC",]
site <- unique(n3$noC)
x <- n3[-100:-1,]
for(i in seq_along(site)){
  m <- n3[n3$noC == site[i] & order(n3$top),]
  m <- m[length(m[,1]),]
  x <- rbind(x,m)
}
names(x)[1] <- "id.p"
n <- n[n$hor!= "BC",]
n <- rbind(n,x)
n <- n[order(n$id.p,n$top),]
n <- n[n$id.p!=500,] # profile 500 has no data
n <- n[n$id.p!=566,] # profile 566 has no data
n <- n[n$id.p!=583,] # profile 583 has no data
n <- n[n$id.p!=591,] # profile 591 has no data
n <- n[n$id.p!=630,] # profile 630 has no data
n <- n[n$id.p!=644,] # profile 644 has no data
n <- n[n$id.p!=662,] # profile 662 has no data
n <- n[n$id.p!=680,] # profile 680 has no data
n <- n[n$id.p!=815,] # profile 815 has no data
n <- n[n$id.p!=837,] # profile 591 has no data
write.csv(n,"calib.data-sp.csv")
write.csv(covar,"calib.data-covar.csv")
