...
d1.1 <- base::merge(x = d1[,c(1:34,42:46)], y = dren, by ="id.p", all.x = TRUE)
...
name(d2.4)
d2.5 <- d2.4[,c(2:9,11:13,22:24,26)]
names(d2.5) <- c("X", "Y", "id.p", "hor", "thick", "n", "tb", 
                 "CEC", "phw", "phkcl", "resistencia", 
                 "OC", "clay", "silt20", "arena.mf")
...
files_n <- list.files(path = "/media/marcos/L0135974_DATA/UserData/BaseARG/COVARIATES/modelling/output/",pattern="mean.tif$")
header_n <- gsub(pattern = "_mean.tif",replacement = "",x = files_n)
...
setwd("/media/marcos/L0135974_DATA/UserData/BaseARG/2_Calibration")
write.csv(calib, "calib.data-3.1.csv")
