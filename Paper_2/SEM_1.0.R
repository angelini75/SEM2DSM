...
names(d)[8:10] <- c("Clay.A", "Clay.B", "Clay.C")
...
ggscatmatM(di[,2:10], columns = 1:9,  alpha=0.15)+  
...
plot(CEC[,2]~CEC[,1], xlim = lim, ylim= lim, xlab = "measured",
       ylab = "predicted", main = "CEC", col = "dark red")
abline(0,1)
...
plot(OC[,2]~OC[,1], xlim = lim, ylim= lim, xlab = "measured",
       ylab = "predicted", main = "OC", col = "dark red")
abline(0,1)
...
plot(Clay[,2]~Clay[,1], xlim = lim, ylim= lim, xlab = "measured",
       ylab = "predicted", main = "Clay", col = "dark red")
abline(0,1)
...
# residuals
colors <- colorRampPalette(c('#E0E0E0', 'black'))(256)
DIF <- abs(S - Sigma.hat)
...
dif <- levelplot(abs(DIF[1:9,9:1]),
                 ...
                 names.attr="SEM",
                 par.settings=list(grid.pars=list(fontfamily="serif")))
s <- levelplot(abs(S[1:9,9:1]),
               col.regions=colors,
               at=seq(0,1.1,0.1),
               xlab = NULL, ylab = NULL,                                                                                                                                                                                                                
               scales=list(x=list(rot=90)),
               par.settings=list(grid.pars=list(fontfamily="serif")))
sigmah <- levelplot(abs(Sigma.hat[1:9,9:1]),
                    col.regions=colors,
                    at=seq(0,1.1,0.1),
                    xlab = NULL, ylab = NULL,                                                                                                                                                                                                                
                    scales=list(x=list(rot=90)),
...
sigmah.lm <- levelplot(abs(Sigma.hat.lm[1:9,9:1]),
                       col.regions=colors,
                       at=seq(0,1.1,0.1),
                       xlab = NULL, ylab = NULL,                                                                                                                                                                                                                
                       scales=list(x=list(rot=90)),
...
tiff(filename = "~/Dropbox/PhD Marcos/Paper 2/Figures/Fig10.tif", 
     width = 2300, height = 1000, res =  250)
update(c(sigmah,s, sigmah.lm),
       scales = list(alternating= FALSE),
       strip = strip.custom(factor.levels = c("SEM", "Observed", "MLR")))
dev.off()


tiff(filename = "~/Dropbox/PhD Marcos/Paper 2/Figures/Fig11.tif", 
     width = 2300, height = 1000, res =  250)
update(c(dif, dif.lm),
       scales = list(alternating= FALSE),
       strip = 
