...
for(i in seq_along(files$Online.Access.URLs)){
  download.file(files$Online.Access.URLs[i],destfile = paste0(granuleID$tile[i], ".zip"),
...
a <- unz(description = paste0(granuleID$tile,".hgz")[1],filename = paste0(granuleID$tile,".zip")[1])
dems <- raster()
unlink(temp)
